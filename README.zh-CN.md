...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/%E5%AE%89%E5%85%A8-Token%20%E9%99%90%E5%88%B6-blue.svg" alt="Security">
</p>
...
### 🔌 2. 安全桥接与 WebSocket (`src/bridge/`)
生产级的通信层实现：
- **Auth Token 闸门**: 通过 `Authorization: Bearer` (HTTP) 或 `?token=` (WS) 提供可选的安全防护。
- **WS 订阅模型**: 基于 Topic（如 `runs`）的发布/订阅系统，实现对 Agent 的实时监控。
- **全流式支持**: 完整支持 Server-Sent Events (SSE) 和 WebSocket 数据流。
...
### 📡 4. CLI 强化模式 (`src/main.js`)
- **Pipe 模式**: 为 CI/CD 和自动化脚本优化的非阻塞执行模式。
- **JSON REPL**: 机器可读的交互模式 (`--json=true`)，便于 IDE 集成。
...
├── bridge/          # 带有 Auth 和订阅机制的 HTTP/WS 安全通信层
├── runtime/         # 执行沙箱与物理隔离逻辑
├── tasks/           # 调度器与任务状态机
...
- [x] **安全桥接层**: 实现 Auth Token 闸门与 WebSocket 订阅模型。
